...
import type { FieldPosition, NestedGameField, NineTupleIndex, OuterGameState } from "./types";
...
const useGameStore = create<GameStore>((setState, getState) => ({
  ...
  checkField: ({ outerGameField, innerGameField }) =>
    setState((state) => {
      ...
      newGame.fields[outerGameField].checkedBy = checkForWinner(
        newGame.fields[outerGameField].fields
      );
...
export const useFieldState = (fieldPosition: FieldPosition) =>
  useGameStore(
    ...
    (oldState, newState) =>
      oldState.active === newState.active && oldState.checkedBy === newState.checkedBy
  );

export const useNestedGameState = (outerGameField: NineTupleIndex) =>
  useGameStore((state) => {
    return { checkedBy: state.game.fields[outerGameField].checkedBy };
  });

export const useGameState = () =>
  useGameStore(({ nextPlayer, game }) => {
